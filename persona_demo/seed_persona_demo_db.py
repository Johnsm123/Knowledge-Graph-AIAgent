...
def _read_main_db() -> dict[str, Any]:
    ...
    lifestyles = kg.run_query("""
        MATCH (m:Member)-[:HAS_LIFESTYLE]->(l:Lifestyle)
        RETURN m.member_id        AS member_id,
               l.bmi              AS bmi,
    ...
    """) or []
    family = kg.run_query("""
        MATCH (m:Member)-[:HAS_RELATIVE]->(fm:FamilyMember)
        OPTIONAL MATCH (fm)-[:HAS_CONDITION]->(c:Condition)
        WITH m, fm, collect(DISTINCT c.name) AS conditions
        RETURN m.member_id        AS member_id,
               fm.relation        AS relation,
               fm.alive           AS alive,
               fm.age_or_age_at_death AS age,
               fm.cause_of_death  AS cause_of_death,
               [x IN conditions WHERE x IS NOT NULL] AS conditions
    """) or []
    history = kg.run_query("""
        MATCH (m:Member)-[:HAS_MEDICAL_HISTORY]->(e:MedicalHistoryEntry)
        RETURN m.member_id  AS member_id,
               e.type       AS type,
               e.label      AS label,
               e.year       AS year,
    ...
    log.info(
        "[MAIN] read %d members, %d claims, %d care-gaps, %d lifestyles, %d family, %d history",
        len(members), len(claims), len(gaps), len(lifestyles), len(family), len(history),
    )
    ...
    lifestyle_by_member: dict[str, dict] = {l["member_id"]: l for l in lifestyles}
    family_by_member: dict[str, list[dict]] = {}
    for f in family:
        family_by_member.setdefault(f["member_id"], []).append(f)
    history_by_member: dict[str, list[dict]] = {}
    for h in history:
        history_by_member.setdefault(h["member_id"], []).append(h)

    return {
        "members": members,
        "claims_by_member": claims_by_member,
        "gaps_by_member": gaps_by_member,
        "lifestyle_by_member": lifestyle_by_member,
        "family_by_member": family_by_member,
        "history_by_member": history_by_member,
    }


# ─── Ideal persona baseline + gap computation ────────────────────────────────

IDEAL_LIFESTYLE = {
    "bmi_range":          [18.5, 24.9],
    "smoking_status":     "Never",
    "alcohol_use":        "None or Occasional",
...
def _compute_lifestyle_gaps(lifestyle: dict) -> list[str]:
    if not lifestyle:
    ...
    if bmi is not None:
        try:
            bmi_f = float(bmi)
            lo, hi = IDEAL_LIFESTYLE["bmi_range"]
            if bmi_f < lo or bmi_f > hi:
                gaps.append(f"BMI {bmi_f} outside ideal {lo}-{hi}")
        except (TypeError, ValueError):
            pass
    smoking = (lifestyle.get("smoking_status") or "").strip()
    if smoking and smoking.lower() not in {"never", "non-smoker", "none"}:
    ...
    if exercise in {"never", "rarely", "none", "sedentary", "1-2 times/week"}:
...
def _compute_family_risk_flags(family: list[dict]) -> list[str]:
    flags: list[str] = []
    for fm in family or []:
...
def _compute_history_gaps(history: list[dict]) -> dict[str, list[str]]:
    immunized = {(h.get("label") or "").strip() for h in history if h.get("type") == "immunization"}
    missing_imm = sorted(RECOMMENDED_IMMUNIZATIONS - immunized)
    unmanaged = [
        (h.get("label") or "").strip()
        for h in history
        if h.get("type") == "current_condition"
        and (h.get("status") or "").strip().lower() in {"uncontrolled", "active", "unmanaged", ""}
    ]
    severe_allergies = [
        (h.get("label") or "").strip()
        for h in history
        if h.get("type") == "allergy" and (h.get("severity") or "").strip().lower() in {"severe", "anaphylaxis"}
    ]
    return {
        "missing_immunizations": missing_imm,
        "unmanaged_conditions":  [u for u in unmanaged if u],
        "severe_allergies":      [s for s in severe_allergies if s],
    }
...
class DemoDB:
    ...
    def seed_member_with_persona(
        self,
        profile: dict,
        evaluation: dict,
        ideal_completion_date: str,
        ...
        family = family or []
        history = history or []
        lifestyle_gaps = _compute_lifestyle_gaps(lifestyle)
        family_flags   = _compute_family_risk_flags(family)
        history_gaps   = _compute_history_gaps(history)
        ...
        self.write(
            ...
            """,
            {
                ...
                "alcohol":  lifestyle.get("alcohol_use", ""),
                ...
                "immunizations": sorted({
                    (h.get("label") or "").strip()
                    for h in history if h.get("type") == "immunization" and h.get("label")
                }),
                "unmanaged":  unmanaged,
        ...
        self.write(
            ...
            SET p.name              = $name,
                ...
                p.pending_count     = 0,
                p.summary           = $summary,
                p.ideal_bmi_range          = $ideal_bmi,
                p.ideal_smoking_status     = $ideal_smoking,
                p.ideal_alcohol_use        = $ideal_alcohol,
                p.ideal_exercise_frequency = $ideal_exercise,
                p.ideal_diet_type          = $ideal_diet,
                p.ideal_sleep_hours        = $ideal_sleep,
                p.ideal_stress_level       = $ideal_stress,
            ...
            """,
            {
                ...
                "summary":          (
                    ...
                    f"({len(applicable)} measures), maintain ideal lifestyle metrics, address all "
                    f"hereditary risks, and keep all immunizations current."
                ),
                "ideal_bmi":      f"{IDEAL_LIFESTYLE['bmi_range'][0]}-{IDEAL_LIFESTYLE['bmi_range'][1]}",
                "ideal_smoking":  IDEAL_LIFESTYLE["smoking_status"],
                "ideal_alcohol":  IDEAL_LIFESTYLE["alcohol_use"],
                "ideal_exercise": IDEAL_LIFESTYLE["exercise_frequency"],
                "ideal_diet":     IDEAL_LIFESTYLE["diet_type"],
                "ideal_sleep":    f"{IDEAL_LIFESTYLE['sleep_hours_avg'][0]}-{IDEAL_LIFESTYLE['sleep_hours_avg'][1]}",
                "ideal_stress":   IDEAL_LIFESTYLE["stress_level"],
                ...
                "severe_allg":         severe_allg,
                "total_links": (
                    len(pending) + len(lifestyle_gaps) + len(missing_imm)
...
def main():
    ...
    try:
        ...
        for member in main_data["members"]:
            ...
            db.seed_member_with_persona(
                member,
                evalr,
                ideal_completion_date=ideal_date,
                lifestyle=main_data["lifestyle_by_member"].get(mid, {}),
                family=main_data["family_by_member"].get(mid, []),
                history=main_data["history_by_member"].get(mid, []),
            )
            n_members += 1
